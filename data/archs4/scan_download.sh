#!/bin/bash
# Phase 1: scan_h5.py — one-time H5 scan (reads H5 once, saves metadata)
# Phase 2: split_data.py — parallel download from H5 (expression only)
#
# Usage:
#   conda activate archs4
#   bash scan_download.sh [h5_jobs] [h5_path] [ref_dir] [output_dir]
#
# Defaults: h5_jobs=2

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
H5="${2:-/media/volume/bulkrnadata/human_gene_v2.latest.h5}"
DATA_DIR="${3:-/media/volume/bulkrnadata/}"
OUTPUT_DIR="${4:-/media/volume/bulkrnadata/humandata/}"
CHUNK_SIZE=5000
H5_JOBS="${1:-2}"
...
python3 -u "$SCRIPT_DIR/scan_h5.py" \
    --datapath "$H5" \
    --refdir "$DATA_DIR" \
...
seq 0 $((N_CHUNKS - 1)) | parallel -j "$H5_JOBS" --line-buffer \
    python3 -u "$SCRIPT_DIR/split_data.py" \
        --datapath "$H5" \
        --outputdir "$OUTPUT_DIR" \
        --chunk_id {} \

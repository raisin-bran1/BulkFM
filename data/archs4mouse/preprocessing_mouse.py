#!/usr/bin/env python3
"""Preprocess a downloaded mouse expression chunk.

Reads:
  - input_chunk_{id}.parquet  (raw counts, rows = genes, cols = samples)
  - gene_symbol_mapping.csv   (precomputed by scan_mouse_h5.py)
  - pc_ensmusg_ids.npy        (precomputed by scan_mouse_h5.py)
  - mouse_exon_lengths.csv    (reference)

Writes:
  - processed_mouse_{id}.parquet  (log1p(TPM), rows = samples, cols = genes)
"""
...
parser = argparse.ArgumentParser(description="Preprocess mouse expression chunk")
...
parser.add_argument("--lengths", type=str, default="mouse_exon_lengths.csv",
                    help="Mouse exon lengths filename (in refdir)")
...
try:
    out_file = os.path.join(args.outputdir, f'processed_mouse_{args.chunk_id:03d}.parquet')
    if os.path.exists(out_file):
        print(f"[{args.chunk_id:03d}] Output already exists, skipping.")
        sys.exit(0)

    # Load precomputed gene metadata (no H5 access)
    mapping_df = pd.read_csv(os.path.join(args.outputdir, 'gene_symbol_mapping.csv'))
    sym_to_ensmusg = dict(zip(mapping_df['symbol'], mapping_df['ensembl_gene_id']))
    shared_gene_pool = set(np.load(
        os.path.join(args.outputdir, 'pc_ensmusg_ids.npy'),
        allow_pickle=True
    ))
    print(f"[{args.chunk_id:03d}] Loaded {len(sym_to_ensmusg)} mappings, {len(shared_gene_pool)} PC genes")
    ...
    exon_lengths = exon_lengths.set_index('gene_symbol')

    def convert_to_TPM(counts_df, length_df):
        L = length_df.reindex(counts_df.index)["exon_length_bp"]
    ...
    # Map symbols -> ENSMUSG, filter to PC genes
    tpm_df['genes'] = tpm_df.index.to_series().map(sym_to_ensmusg)

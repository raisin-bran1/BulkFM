...
def _coerce_config_types(cfg: dict) -> None:
    ...
    if cfg.get('dynamic_mask_range') is not None:
        if isinstance(cfg['dynamic_mask_range'], str):
            parts = cfg['dynamic_mask_range'].replace('(', '').replace(')', '').split(',')
            try:
                cfg['dynamic_mask_range'] = (float(parts[0]), float(parts[1]))
            except (ValueError, IndexError):
                cfg['dynamic_mask_range'] = None
